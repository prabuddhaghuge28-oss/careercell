...
function ViewUserData() {
  document.title = 'CarrerCell | User Details';
    ...
    const fetchAllData = async () => {
      try {
        ...
        if (studentDataRes.data?.studentProfile?.appliedJobs) {
          ...
          // Derive counts from job applicant subdocuments where possible (legacy 'status' removed)
          const interviewCount = appliedJobs.filter((app) => {
            const applicant = app.jobId?.applicants?.find(a => String(a.studentId) === String(studentDataRes.data._id));
            return !!(applicant && applicant.currentRound);
          }).length;

          const rejectCount = appliedJobs.filter((app) => {
            const applicant = app.jobId?.applicants?.find(a => String(a.studentId) === String(studentDataRes.data._id));
            return applicant?.roundStatus === 'failed';
          }).length;

          // Heuristic: treat a placement as when applicant has a selectionDate
          const hiredJob = appliedJobs.find((app) => {
            const applicant = app.jobId?.applicants?.find(a => String(a.studentId) === String(studentDataRes.data._id));
            return !!applicant?.selectionDate;
          });
          ...
          setPlacement({
            ...
            jobId: hiredJob ? (hiredJob.jobId?._id || null) : null,
          });
        }
                ...
                <div className="grid grid-cols-2 max-md:grid-cols-1">
                  ...
                  <div className="flex flex-col justify-start items-end max-md:items-center max-md:mt-1">
                    ...
                    <Col xs={6} md={9} className=" flex justify-end rounded">
                      <Image src={SERVER_BASE_URL + userData?.profile} thumbnail />
                    </Col>
                    {
                      (userData?.studentProfile?.resume?.filepath !== "undefined" && userData?.role === 'student') && (
                        <div className="py-2 px-2  max-sm:text-sm">
                          <span className='bg-blue-500 py-1 pr-2 rounded cursor-pointer hover:bg-blue-700'>
                            <a href={SERVER_BASE_URL + userData?.studentProfile?.resume?.filepath} target='_blanck' className='no-underline text-white'>
                                          ...
                                          <tbody>
                                            {
                                              userData?.studentProfile?.appliedJobs?.length > 0 ? (
                                                  ...
                                                  return (
                                                    <>
                                                      <tr key={index}>
                                                        ...
                                                        <td>
                                                          {job?.appliedAt ? new Date(String(job?.appliedAt).split('T')[0]).toLocaleDateString('en-IN') : '-'}
                                                        </td>
                                                      </tr>
                                                    </>
                                                  )
                                                ...
                                                <tr>
                                                  <td colSpan={4}>Not Yet Applied Any Job!</td>
                                                </tr>
                                              )
                                            }
                      ...
                      <div className=''>
                        ...
                        <div className="grid gap-1">
                          ...
                          <div className="grid grid-flow-col">
                            <div className="space-y-4">
                              {
                                userData?.studentProfile?.ABCID && (
                                    <div>
                                      <span className="text-gray-700 font-bold">ABCID: </span>
                                      <span className="text-gray-800">
                                        {userData?.studentProfile?.ABCID}
                                      </span>
                                    </div>
                                      ...
                                      <div className="space-y-6 max-sm:space-y-3">
                                        {
                                          userData?.studentProfile?.SGPA?.sem1 && (
                                            <div className='flex flex-col gap-1'>
                                              ...
                                              {
                                                userData?.studentProfile?.resultFiles?.sem1?.filepath && 
                                                userData?.studentProfile?.resultFiles?.sem1?.filepath !== "undefined" && (
                                                  <div className="text-xs">
                                                    <a href={SERVER_BASE_URL + userData?.studentProfile?.resultFiles?.sem1?.filepath} target='_blank' rel='noopener noreferrer' className='text-blue-600 hover:text-blue-800 no-underline'>
                                                      <i className="fa-regular fa-file-pdf px-1" />
                                                      View Sem I Result
                                                    </a>
                                                  </div>
                                                )
                                        ...
                                        }
                                        {
                                          userData?.studentProfile?.SGPA?.sem2 && (
                                            <div className='flex flex-col gap-1'>
                                              ...
                                              {
                                                userData?.studentProfile?.resultFiles?.sem2?.filepath && 
                                                userData?.studentProfile?.resultFiles?.sem2?.filepath !== "undefined" && (
                                                  <div className="text-xs">
                                                    <a href={SERVER_BASE_URL + userData?.studentProfile?.resultFiles?.sem2?.filepath} target='_blank' rel='noopener noreferrer' className='text-blue-600 hover:text-blue-800 no-underline'>
                                                      <i className="fa-regular fa-file-pdf px-1" />
                                                      View Sem II Result
                                                    </a>
                                                  </div>
                                        ...
                                        }
                                        {
                                          userData?.studentProfile?.SGPA?.sem3 && (
                                            <div className='flex flex-col gap-1'>
                                              ...
                                              {
                                                userData?.studentProfile?.resultFiles?.sem3?.filepath && 
                                                userData?.studentProfile?.resultFiles?.sem3?.filepath !== "undefined" && (
                                                  <div className="text-xs">
                                                    <a href={SERVER_BASE_URL + userData?.studentProfile?.resultFiles?.sem3?.filepath} target='_blank' rel='noopener noreferrer' className='text-blue-600 hover:text-blue-800 no-underline'>
                                                      <i className="fa-regular fa-file-pdf px-1" />
                                                      View Sem III Result
                                                    </a>
                                                  </div>
                                        ...
                                        }
                                        {
                                          userData?.studentProfile?.SGPA?.sem4 && (
                                            <div className='flex flex-col gap-1'>
                                              ...
                                              {
                                                userData?.studentProfile?.resultFiles?.sem4?.filepath && 
                                                userData?.studentProfile?.resultFiles?.sem4?.filepath !== "undefined" && (
                                                  <div className="text-xs">
                                                    <a href={SERVER_BASE_URL + userData?.studentProfile?.resultFiles?.sem4?.filepath} target='_blank' rel='noopener noreferrer' className='text-blue-600 hover:text-blue-800 no-underline'>
                                                      <i className="fa-regular fa-file-pdf px-1" />
                                                      View Sem IV Result
                                                    </a>
                                                  </div>
                                      ...
                                      <div className="space-y-6 max-sm:space-y-3">
                                        {
                                          userData?.studentProfile?.SGPA?.sem5 && (
                                            <div className='flex flex-col gap-1'>
                                              ...
                                              {
                                                userData?.studentProfile?.resultFiles?.sem5?.filepath && 
                                                userData?.studentProfile?.resultFiles?.sem5?.filepath !== "undefined" && (
                                                  <div className="text-xs">
                                                    <a href={SERVER_BASE_URL + userData?.studentProfile?.resultFiles?.sem5?.filepath} target='_blank' rel='noopener noreferrer' className='text-blue-600 hover:text-blue-800 no-underline'>
                                                      <i className="fa-regular fa-file-pdf px-1" />
                                                      View Sem V Result
                                                    </a>
                                                  </div>
                                        ...
                                        }
                                        {
                                          userData?.studentProfile?.SGPA?.sem6 && (
                                            <div className='flex flex-col gap-1'>
                                              ...
                                              {
                                                userData?.studentProfile?.resultFiles?.sem6?.filepath && 
                                                userData?.studentProfile?.resultFiles?.sem6?.filepath !== "undefined" && (
                                                  <div className="text-xs">
                                                    <a href={SERVER_BASE_URL + userData?.studentProfile?.resultFiles?.sem6?.filepath} target='_blank' rel='noopener noreferrer' className='text-blue-600 hover:text-blue-800 no-underline'>
                                                      <i className="fa-regular fa-file-pdf px-1" />
                                                      View Result
                                                    </a>
                                                  </div>
                              ...
                              <>
                                <div className="">
                                  ...
                                  <div className="space-y-1 pl-2">
                                    ...
                                    {
                                      userData?.studentProfile?.resultFiles?.ssc?.filepath && 
                                      userData?.studentProfile?.resultFiles?.ssc?.filepath !== "undefined" && (
                                        <div className="mt-2">
                                          <a href={SERVER_BASE_URL + userData?.studentProfile?.resultFiles?.ssc?.filepath} target='_blank' rel='noopener noreferrer' className='bg-green-500 py-1 px-2 rounded cursor-pointer hover:bg-green-700 text-white text-sm no-underline inline-block'>
                                            <i className="fa-regular fa-file-pdf px-1" />
                                            View SSC Result
                                          </a>
                                          <p className='text-xs text-gray-500 mt-1'>{userData?.studentProfile?.resultFiles?.ssc?.filename}</p>
                                        </div>
                                      )
                                    }
                                  </div>
                            ...
                            <div className="space-y-1 pl-2">
                              {
                                ...
                                userData?.studentProfile?.pastQualification?.hsc?.board !== "undefined" && 
                                userData?.studentProfile?.pastQualification?.hsc?.board !== "" ? (
                                  <div>
                                    <span className="text-gray-700 font-bold">Board: </span>
                                    <span className="text-gray-800">
                              ...
                              }
                              {
                                ...
                                userData?.studentProfile?.pastQualification?.hsc?.year !== "undefined" && 
                                userData?.studentProfile?.pastQualification?.hsc?.year !== "" ? (
                                  <div>
                                    <span className="font-bold text-gray-700">Passing Year: </span>
                                    <span className="text-gray-800">
                              ...
                              }
                              {
                                ...
                                userData?.studentProfile?.pastQualification?.hsc?.percentage !== "undefined" && 
                                userData?.studentProfile?.pastQualification?.hsc?.percentage !== "" ? (
                                  <div>
                                    <span className="font-bold text-gray-700">Percentage: </span>
                                    <span className="text-gray-800">
                                ...
                                (!userData?.studentProfile?.pastQualification?.hsc?.percentage || 
                                 ...
                                 userData?.studentProfile?.pastQualification?.hsc?.percentage === "") && (
                                  <div className="text-gray-500 italic text-sm">
                                    HSC information not provided
                                  </div>
                                )
                              }
                              {
                                userData?.studentProfile?.resultFiles?.hsc?.filepath && 
                                userData?.studentProfile?.resultFiles?.hsc?.filepath !== "undefined" && (
                                  <div className="mt-2">
                                    <a href={SERVER_BASE_URL + userData?.studentProfile?.resultFiles?.hsc?.filepath} target='_blank' rel='noopener noreferrer' className='bg-green-500 py-1 px-2 rounded cursor-pointer hover:bg-green-700 text-white text-sm no-underline inline-block'>
                                      <i className="fa-regular fa-file-pdf px-1" />
                                      View HSC Result
                                    </a>
                                    <p className='text-xs text-gray-500 mt-1'>{userData?.studentProfile?.resultFiles?.hsc?.filename}</p>
                                  </div>
                                )
                              }
                            </div>
