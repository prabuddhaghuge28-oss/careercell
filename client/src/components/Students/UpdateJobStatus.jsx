...
function UpdateJobStatus() {
  document.title = 'CarrerCell | Update Job Application Status';
  ...
  // check if hired 
  // check if selected (was previously 'hired')
    ...
    axios.get(`${BASE_URL}/user/detail`, {
      ...
      .then(res => {
        setCurrentUser({
          ...
          role: res.data.role,
          ABCID: res.data.studentProfile.ABCID,
        });
      })
      .catch(err => {
  ...
  const fetchJobDetailsOfApplicant = async () => {
    if (data?.applicants?.length !== 0) {
      ...
      if (appliedApplicant) setApplicant(appliedApplicant) // If no applicant found, navigate to 404
          ...
          // if package exists then set selected-state to show package input
          if (appliedApplicant?.package) setIsSelected(true);
    }
  }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (applicant?.package) setIsSelected(true);
        setLoading(false);
      } catch (error) {
                    ...
                    <Accordion.Body>
                      <div className="">
                        <div className="grid grid-cols-2 gap-4 max-sm:grid-cols-1">
                              ...
                              <div className="cursor-pointer w-fit flex gap-1 justify-between items-center">
                                ...
                                >
                                  <a
                                    ...
                                    target="_blanck"
                                    href={SERVER_BASE_URL + applicant?.offerLetter}
                                  >
                                    {
                                      eyeIsHover ? (
                          ...
                          {/* job status removed from UI */}
                          {
                            isSelected === true && (
                              <div className="col-span-2">
                                {/* package input for selected applicants */}
