...
const AppliedToJob = async (req, res) => {
  ...
  // Prevent duplicate application: check both user record and job applicants
  if (user?.studentProfile?.appliedJobs?.some(job => job.jobId == req.params.jobId)) return res.json({ msg: "Already Applied!" });
  if (job?.applicants?.some(a => String(a.studentId) === String(user._id))) return res.json({ msg: "Already Applied!" });

    if (!user?.studentProfile?.resume?.filename) return res.json({ msg: 'Please Upload Resume First, Under "Placements" > "Placement Profile"' });

    user?.studentProfile?.appliedJobs?.push({ jobId: req.params.jobId, appliedAt: Date.now() });
